...
public class ImsResolver implements ImsServiceController.ImsServiceControllerCallbacks {
    ...
    public ImsResolver(Context context, String defaultMmTelPackageName,
        ...
        // Iceows patch
        defaultMmTelPackageName="com.huawei.ims";
        Log.i(TAG, "device MMTEL package huawei: " + defaultMmTelPackageName + ", device RCS package:"
                + defaultRcsPackageName);
    ...
    private boolean isDeviceService(ImsServiceInfo info) {
        Log.i(TAG, "iceows - isDeviceService enter");
        if (info == null) return false;
        Log.i(TAG, "iceows - isDeviceService packagename :" + info.name.getPackageName());

        for (Integer i : mDeviceServices.keySet()) {
            Log.i(TAG, " keyset : " + ImsFeature.FEATURE_LOG_MAP.get(i) + " -> " + mDeviceServices.get(i));
        }        
    ...
    private void scheduleQueryForFeatures(ImsServiceInfo service, int delayMs) {
    	...
    	Log.i(TAG, "scheduleQueryForFeatures: dynamic query enter ");
    
        if (service == null) {
            return;
        }
        
        Log.i(TAG, "scheduleQueryForFeatures: dynamic query for " + service.name);
    ...
    private void startDynamicQuery(ImsServiceInfo service) {
        ...
        Log.d(TAG, "iceows - startDynamicQuery");
    ...
    private List<ImsServiceInfo> searchForImsServices(String packageName,
        ...
        for (ResolveInfo entry : packageManager.queryIntentServicesAsUser(
            ...
            if (serviceInfo != null) {
                ...
                Log.i(TAG, "service name: " + serviceInfo.name + " packagename: " + serviceInfo.packageName);
                ...
                if (isDeviceService(info)
                    ...
                    Log.i(TAG, "service name: " + info.name + " try metadata1");
                    if (serviceInfo.metaData != null) {
                        Log.i(TAG, "service name: " + info.name + " try metadata not null");
                        if (serviceInfo.metaData.getBoolean(METADATA_MMTEL_FEATURE, false)) {
                            ...
                            Log.i(TAG, "service name: " + info.name + " add feature");
